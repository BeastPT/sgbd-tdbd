    ...
    if (!array_key_exists("estado", $_REQUEST)) {
        ...
        if (mysqli_num_rows($verifica_subitens) == 0) {
            ...
            while ($tipo_de_unidade_rows = mysqli_fetch_assoc($tipo_de_unidade)) {
                echo "<option value=" . $tipo_de_unidade_rows['name'] . ">" . $tipo_de_unidade_rows['name'] . "</option>";
            }
            echo "</select></br>";
            echo "
                Ordem do campo no formulário: <input type='text' name=form_field_order></br>
                Obrigatório:</br>";
            echo "<input type='radio' name=mandatory value=1>Sim</br>";
            echo "<input type='radio' name=mandatory value=0>Não</br>";
        ...
        if (empty($_REQUEST['subitem_name'])) {
        }
    }
}
